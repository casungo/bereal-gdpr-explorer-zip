...
const postsByMonth = $derived(() => {
	return filteredPosts().reduce(
		(acc, post) => {
			const monthKey = format(startOfMonth(new Date(post.takenAt)), "yyyy-MM");
			if (!acc[monthKey]) {
				acc[monthKey] = [];
			}
          ...
          >
            ...
            >
              ...
              >
                ...
                <span
                  class="ml-2 text-sm font-normal text-base-content/70"
                >
                  ({monthPosts.length}
                  posts)
                </span>
              ...
              >
                {#each monthPosts as post (post.id)}
                  ...
                  >
                    ...
                    <div
                      class="card-body p-4 flex flex-col flex-1"
                    >
                      <div
