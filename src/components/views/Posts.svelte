<script lang="ts">
  ...
  import MediaPreview from "@/components/ui/MediaPreview.svelte";
  import { format, formatDistanceToNow, startOfMonth } from "date-fns";
  import { Camera, Download, RefreshCcw, Eye, Video } from "@lucide/svelte";
  ...
  const sortOptions: SortOption[] = [
    ...
    {
      value: "lateInSeconds-asc",
      label: "On Time First",
    },
    {
      value: "lateInSeconds-desc",
      label: "Latest First",
    },
  ];
  ...
  function handleDownloadPost(e: MouseEvent, post: Post) {
    ...
    downloadDialogState = {
      open: true,
      posts: [post],
      defaultName: `bereal-post-${format(new Date(post.takenAt), "yyyy-MM-dd")}`,
    };
  }
...
{#if posts.length === 0}
  <EmptyState
    ...
    description="This export does not include regular BeReal posts."
  />
{:else}
  <div class="space-y-4">
    <FilterBar
        ...
        <div class="card bg-base-100 shadow-xl overflow-hidden">
          ...
          >
            <div class="flex min-w-0 items-center gap-2">
              ...
              ></div>
              <h3 class="min-w-0 text-base font-semibold sm:text-lg">
            ...
            <div class="p-4 pt-0">
              ...
              >
                {#each monthPosts as post (post.id)}
                  {@const primaryUrl = post.primary?.path
                    ? media[post.primary.path]
                    : undefined}
                  {@const secondaryUrl = post.secondary?.path
                    ? media[post.secondary.path]
                    : undefined}
                  {@const videoUrl = post.btsMedia
                    ? media[post.btsMedia.path]
                    : null}
                  {@const imageAspectRatio =
                    post.primary?.width && post.primary?.height
                      ? post.primary.width / post.primary.height
                      : 3 / 4}
                  {@const videoAspectRatio = post.btsMedia
                    ? post.btsMedia.width / post.btsMedia.height
                    : 16 / 9}
                  <div
    ...
    {#if filteredPosts.length === 0}
      <EmptyState
        ...
        description="Adjust the date range or audience filter to bring posts back."
