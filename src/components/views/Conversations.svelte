...
function isValidMessage(content: string): boolean {
	...
	const hasInvalidChars = /[\x00-\x08\x0B\x0C\x0E-\x1F\x7F-\x9F]/.test(content);

	const specialCharCount = (content.match(/[^\w\s\p{P}\p{S}]/gu) || []).length;
...
{:else}
  ...
  >
    <div
      class="lg:col-span-1 h-full overflow-hidden"
    >
      <div
        class="card bg-base-100 shadow-xl h-full flex flex-col"
      >
        <div
