<script lang="ts">
  ...
  const frequencyChartData: FrequencyChartData[] = $derived(
  ...
  const latePosts = $derived(
    allPosts.filter((p: Post | Memory) => {
  ...
  const onTimePercentage = $derived(
  ...
  const topRealmojis: TopRealmoji[] = $derived(
    Object.entries(realmojiCounts)
      .map(([name, value]) => ({
        name,
    ...
    <div class="stats bg-base-100 shadow">
      <div class="stat">
        ...
        <div class="stat-value text-2xl">
          {topRealmojis[0]?.name || "N/A"}
        </div>
        <div class="stat-desc">
          {topRealmojis[0]?.value || 0}
          times
        </div>
      </div>
    </div>
  ...
  <div class="grid gap-6 md:grid-cols-1 lg:grid-cols-2">
      ...
      <div class="card-body">
        ...
        {#if frequencyChartData.length > 0}
          <div class="w-full overflow-x-auto pb-6">
            <div
              class="flex min-w-max items-end justify-start gap-3 px-2 pt-4 sm:justify-center"
            >
              {#each frequencyChartData as item}
                <div class="flex w-12 flex-col items-center gap-2">
                  <div
                    class="w-8 rounded-t bg-primary transition-all duration-300 hover:bg-primary/80"
        ...
        {:else}
          <EmptyState
            ...
            title="No posting activity yet"
            description="Posts and memories will appear here when they are present in the export."
          />
        {/if}
      </div>
    </div>
      ...
      <div class="card-body">
            ...
            <div class="relative h-48 w-48">
              <div
                ...
                style="background: conic-gradient(
								var(--color-success) 0deg {onTimePercentage * 3.6}deg,
								var(--color-error) {onTimePercentage * 3.6}deg 360deg
							)"
              ></div>
              <div
        ...
        {:else}
          <EmptyState
            ...
            description="Timing insights need posts or memories with capture timestamps."
          />
        {/if}
      </div>
    </div>
    ...
    <div class="card bg-base-100 shadow-xl">
      <div class="card-body">
        ...
        {#if posts.length > 0 && Object.entries(visibilityCounts).length > 0}
          <div class="space-y-4">
        ...
        {:else}
          <EmptyState
            ...
            description="Audience insights need regular post records in the export."
          />
        {/if}
      </div>
    </div>
      ...
      <div class="card-body">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
          ...
          {:else}
            <EmptyState
              icon={SmilePlus}
              title="No realmojis found"
              description="Reaction insights will appear when realmoji data is present."
            />
          {/if}
